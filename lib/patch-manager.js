...
export class ManagedPatch {
  ...
  sync(ids) {
    ...
    // 移除之前 sync([]) 写入的 "[]" 占位行——有实际条目时它会破坏 YAML
    // 顶层结构（[] 是完整的空数组，后续 - id 条目会被解析器拒绝）。
    out = out.filter((line) => line.trim() !== "[]");
    // 关键：dsh 要求 cordis.patch.yml 顶层必须是 YAML 数组。若清空 managed
    // block 后文件只剩注释（解析结果为 null），dsh 启动会直接抛
    // "must be a top-level YAML array"。此时补一个空数组占位。
    const hasContent = out.some((line) => {
      const t = line.trim();
      return t !== "" && !t.startsWith("#");
    });
    if (!hasContent) out = ["[]", ...block];

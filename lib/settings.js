...
export async function registerSettings(ctx, entryConfig, onChange) {
  ...
  const base = { policy: policy.mode, autoDisable: policy.autoDisable, autoPatchProfile: policy.autoPatchProfile, installGate: policy.installGate, allowlist: policy.allowlist };
  let current = () => base;

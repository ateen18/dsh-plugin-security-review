...
export function makeSettingsSchema(z) {
  return z.object({
    ...
    autoPatchProfile: z.boolean().default(true),
...
export function normalizePolicy(section) {
  ...
  return {
    mode,
    autoDisable: section?.autoDisable !== false,
    autoPatchProfile: section?.autoPatchProfile !== false,
...
export async function registerSettings(ctx, entryConfig, onChange) {
  ...
  const base = { policy: policy.mode, autoDisable: policy.autoDisable, autoPatchProfile: policy.autoPatchProfile, allowlist: policy.allowlist };
  let current = () => base;

...
export function scopeOfResolved(resolvedPath, baseUrl) {
  ...
  // 仅在能确认 dsh 安装根（从而能排除官方插件）时，才把「声明了 dsh.* 的包」纳入审查；
  // 否则只审 profile 目录内的包——防止宿主解析失败时把官方插件误判为可审对象。
  if (dshInstallRoot && isDshPlugin) return { packageDir, name: pkg.name ?? null };
  return null;
}
...
export async function installImportGate() {
  ...
  EntryTree.prototype.import = async function gatedImport(name, getOuterStack) {
    try {
      ...
      if (blocked) {
        // 以 no-op 插件模块替代真实模块：不执行其任何代码，同时保证加载流程
        // （HMR 组事务/动态 create）不因 import 抛错而失败，杜绝崩溃链
        gateState.log("warn", "已拦截插件导入 " + name + "（以空实现替代，不执行其代码）：" + blocked.summary + " (报告: " + blocked.reportPath + ")");
        return { default: function securityReviewBlockedNoop() {} };
      }
    } catch (error) {
...
async function gateBeforeImport(tree, name) {
  ...
  let resolved = null;
  try { resolved = resolveModulePath(tree?.ctx?.baseUrl, name); } catch {}
  if (!resolved) return null;
  let scope = null;
  try { scope = scopeOfResolved(resolved, tree?.ctx?.baseUrl); } catch {}
...
export async function decideFor(scope, opts = {}) {
  ...
  // blocked 表示「恶意判定」本身；是否真正禁用/拦截由调用方结合策略决定

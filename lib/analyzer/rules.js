...
import { decodePayloads, scanDecodedPayloads, stripJsLiterals } from "./decode.js";
...
export const RULES = [
  ...
  },
  {
    ...
    check(ctx, file, text) {
      if (!file) return;
      // 排除文档类文件与示例域名，降低注释/README 误报
      if (file.ext === ".md" || file.ext === ".txt" || file.ext === ".html" || file.ext === ".htm") return;
      runRegexRule(this, ctx, file, text, /['"`]http:\/\/(?!localhost|127\.0\.0\.1|0\.0\.0\.0|\[::1\]|example\.com|example\.org)[^'"`\s]+/g);
    }
  },
  {
    id: "dsh-host-abuse",
  ...
  },
  {
    ...
    severity: "low",
    category: "data-exfiltration",
    message: "硬编码加密货币地址格式（可能是钱包/挖矿载荷，也可能是哈希/测试常量）",
    recommendation: "确认为测试数据或哈希常量；生产代码中不应出现钱包地址",
    check(ctx, file, text) {
      if (!file) return;
      // 排除文档类文件（0x+40hex 与钱包前缀在文档/注释中误报极高）
      if (file.ext === ".md" || file.ext === ".txt" || file.ext === ".html" || file.ext === ".htm") return;
  ...
  },
  {
    ...
    severity: "critical",
    category: "obfuscation",
    message: "反混淆解码：源码中的编码载荷（base64/hex/fromCharCode/\\xNN）解码后命中密钥、命令或网络地址",
    recommendation: "解码内容需人工确认；含密钥/反弹 shell/下载执行等即视为恶意信号",
    check(ctx, file, text) {
      if (!file || !JS_EXTS.has(file.ext)) return;
      const payloads = decodePayloads(text);
      if (!payloads.length) return;
      const findings = scanDecodedPayloads(payloads);
      for (const f of findings.slice(0, MAX_PER_RULE_FILE)) {
        ctx.add(makeFinding(this, ctx, file, f.index, text, { severity: f.severity, message: f.message }));
      }
    }
  },
  {
    ...
    severity: "high",
    category: "dangerous-code",
    message: "词法剥离后仍存在动态拼接的危险调用（exec/eval/require 参数含变量或拼接），是混淆代码的常见形态",
    recommendation: "确认调用参数来源可信；拒绝不可信输入进入命令/动态执行",
    check(ctx, file, text) {
      if (!file || !JS_EXTS.has(file.ext)) return;
      const code = stripJsLiterals(text);
      runRegexRule(this, ctx, file, code, /(?:\b|\s)(exec|execSync|spawn|spawnSync|eval|new\s+Function)\s*\(\s*[^)]*\$\{|(?:exec|eval|require)\s*\(\s*[a-zA-Z_$][\w$]*\s*\)/g, (m) => {
        const windowText = code.slice(m.index, m.index + 120);
        return { severity: /(eval|new\s+Function)/.test(m[1]) ? "critical" : "high", message: "动态拼接调用 " + m[1] + "：" + windowText.trim().slice(0, 80) };
      });
    }
  },
];

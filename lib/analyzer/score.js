...
/**
 * 规则 id -> 人类可读的直观风险短语（供 Web UI / 摘要展示）。
 * 未列出的规则 id 在摘要中忽略（多为低危/信息级辅助发现）。
 */
export const RISK_LABELS = {
  "install-script": "安装脚本自动执行",
  "download-exec": "下载后执行恶意脚本",
  "danger-child-process": "执行系统命令",
  "dynamic-eval": "动态执行/混淆代码",
  "remote-code-load": "远程代码加载",
  "data-exfil": "凭据/数据外传",
  "hardcoded-secret": "硬编码密钥（API 密钥泄露）",
  "obfuscation": "代码混淆",
  "unsafe-fs": "读写用户文件",
  "native-binary": "原生二进制（无法静态审查）",
  "prototype-pollution": "原型污染",
  "typosquat": "可疑包名（疑似钓鱼）",
  "suspicious-deps": "可疑依赖",
  "plain-http": "明文网络通信",
  "dsh-host-abuse": "可操控插件清单",
  "miner-wallet": "加密货币地址/挖矿载荷",
  "client-browser-risk": "浏览器端数据泄露",
  "integrity-mismatch": "安装包完整性校验失败",
  "review-error": "审查器运行失败"
};

const RISK_SEVERITY_ORDER = { critical: 0, high: 1, medium: 2, low: 3, info: 4 };

/**
 * 把 findings 聚合为一句话直观风险摘要，例如：
 *   "有风险（评分 46）— 浏览器端数据泄露、动态执行/混淆代码"
 * 取严重级别最高、且规则 id 有映射短语的前 N 项（默认 3）。
 */
export function summarizeRisks(findings, verdict, score) {
  const sorted = [...(findings ?? [])].sort((a, b) =>
    (RISK_SEVERITY_ORDER[a.severity] ?? 9) - (RISK_SEVERITY_ORDER[b.severity] ?? 9)
  );
  const scoreText = typeof score === "number" ? "（评分 " + score + "）" : "";
  const serious = sorted.filter((f) => f.severity === "critical" || f.severity === "high");
  const notes = sorted.filter((f) => f.severity === "medium" || f.severity === "low");
  const labelsOf = (list, max) => {
    const out = [];
    const seen = new Set();
    for (const f of list) {
      if (out.length >= max) break;
      const label = RISK_LABELS[f.id];
      if (label && !seen.has(label)) { seen.add(label); out.push(label); }
    }
    return out;
  };
  // 结论与摘要语义对齐：block/warn 只列严重发现；pass 不再罗列风险词
  if (verdict === "block") {
    const labels = labelsOf(serious.length ? serious : sorted, 3);
    return "危险" + scoreText + (labels.length ? " — " + labels.join("、") : "");
  }
  if (verdict === "warn") {
    const labels = labelsOf(serious, 3);
    return "有风险" + scoreText + (labels.length ? " — " + labels.join("、") : "");
  }
  if (verdict === "audit") return "仅审计" + scoreText;
  if (notes.length) return "安全" + scoreText + "（存在 " + notes.length + " 条中低危提示，详见报告）";
  return "安全" + scoreText;
}

...
export function summarizeRisks(findings, verdict, score) {
  ...
  const prefix = verdict === "block" ? "危险" : verdict === "warn" ? "有风险" : verdict === "audit" ? "仅审计" : "安全";
  const scoreText = typeof score === "number" ? "（评分 " + score + "）" : "";
  if (!labels.length) return prefix + scoreText;
  return prefix + scoreText + " — " + labels.join("、");
}
